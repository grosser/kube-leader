...
// Original:
// https://raw.githubusercontent.com/operator-framework/operator-sdk/master/pkg/leader/leader.go
// Modifications:
// - silent when waiting to become leader
// - remove global kubernetes log reference
// - fixed stale cache issue https://github.com/operator-framework/operator-sdk/pull/3059
...
func Become(ctx context.Context, lockName string) error {
	...
	for {
		...
		case apierrors.IsAlreadyExists(err):
			// refresh the lock so we use current leader in messages
			key := crclient.ObjectKey{Namespace: ns, Name: lockName}
			...
			default:
				...
				default:
					//log.Info("Not the leader. Waiting.") // KEEP THIS MODIFICATION
				}
			}
